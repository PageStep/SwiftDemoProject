...
extension PhotoBrowserViewCell {
    fileprivate func setupContent(picURL : NSURL?) {
        ...
        imageView.sd_setImage(with: getBigURL(smallURL: picURL) as URL!, placeholderImage: image, options: [], progress: { (current, total) -> Void in
    ...
    fileprivate func getBigURL(smallURL : NSURL) -> NSURL {
